...
{
    $sql_user = "SELECT user_id,firstname,lastname,email_address,device_id,join_date,social_id,login_type,device_type,status,role,category_id,profile_photo FROM app_users WHERE user_id='" . $user_id . "' ";
    try {
        $dbCon = getConnection();
        $stmt  = $dbCon->query($sql_user);
        $stmt->execute();
