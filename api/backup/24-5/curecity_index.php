...
function addReviewLike()
{
    ...
    try {
		...
		if(empty($check_array)){
			...
			$stmt = $dbCon->query($sql);
			$review_count_array  = $stmt->fetchObject();
			
			if(!empty($review_count_array->review_likes)){
				
				$like_array = array_filter(explode(',',$review_count_array->review_likes));
				$review_like_count = count($like_array);
...
function removeReplyLike($id)
{
    ...
    $sql = "SELECT reply_likes FROM review_reply  WHERE id  = '".$reply_id."'";
    try {
        $dbCon = getConnection();
		...
		$sql = "UPDATE review_reply SET `reply_likes` =  '".$reply_likes."' WHERE id  = '".$reply_id."'";
		...
		$sql = "SELECT reply_likes FROM review_reply WHERE id = '".$reply_id."'";
		$stmt = $dbCon->query($sql);
		$reply_count_array  = $stmt->fetchObject();
		
		if(!empty($reply_count_array->reply_likes)){
			
			$like_array = array_filter(explode(',',$reply_count_array->reply_likes));
			$reply_like_count = count($like_array);
    ...
    try {
        ...
        if (!empty($category)) {
            ...
            foreach ($category as $category_value) {
                ...
                $sql_query                 = "SELECT id,location_name FROM app_users AS au INNER JOIN location AS l ON l.id = au.current_location_id WHERE au.status = 'Verified' AND au.category_type_id = '" . $category_value->id . "' AND l.status = '1' GROUP BY id";
                $stmt                      = $dbCon->query($sql_query);
                $location_array            = $stmt->fetchAll(PDO::FETCH_OBJ);
                $location_extra_key    	   = (object) array(
												"id" => '0',
                ...
                // for speciality
                $sql_query                 = "SELECT id,category_name FROM app_users AS au INNER JOIN category AS c ON c.id = au.category_id WHERE au.status = 'Verified' AND au.category_type_id = '" . $category_value->id . "' GROUP BY id ";
                $stmt                      = $dbCon->query($sql_query);
                $speciality_array          = $stmt->fetchAll(PDO::FETCH_OBJ);
                $speciality_extra_key    	   = (object) array(
												"id" => '0',
				 ...
				 array_unshift($speciality_array, $speciality_extra_key); 
                // for name
                $sql_query                 = "SELECT user_id,firstname FROM app_users WHERE status = 'Verified' AND category_type_id = '" . $category_value->id . "'";
                $stmt                      = $dbCon->query($sql_query);
                ...
                // for exp
                $sql_query                 = "SELECT MAX(practicing_since) AS exp_max ,MIN(practicing_since) AS exp_min FROM app_users WHERE status = 'Verified' AND category_type_id = '" . $category_value->id . "' ";
                $stmt                      = $dbCon->query($sql_query);
    ...
    try {
        ...
        if(!empty($social_id)){
			...
			if(!empty($profile_photo)){
					...
					$name = $txt.time();
					$profile_photo = $name.".".$ext;
					$upload = file_put_contents($_SERVER["DOCUMENT_ROOT"] ."/curecity/uploads/".$profile_photo,file_get_contents($url));
			...
			if(!empty($user_check)){
			   ...
			   $sql_update = "UPDATE app_users SET `profile_photo` = '".$profile_photo."' WHERE `user_id` = '".$user_check[0]->user_id."'";
			   $stmt_update = $dbCon->prepare($sql_update);  
			   $stmt_update->execute();
				...
				$sql  = "INSERT INTO app_users SET `mobile` = '".$mobile."',`firstname` = '" . $firstname . "',`lastname` = '" . $lastname . "', `email_address`= '" . $email_address . "', `password`= '" . $password . "', `social_id`= '" . $social_id . "',`join_date` = '" . TODAY_DATE_TIME . "',`modified_date` = '" . TODAY_DATE_TIME . "',`role`= '" . $role . "',`category_id`= '" . $category_id . "',`login_type` = '" . $login_type . "',`device_type` = '" . $device_type . "',`device_id` = '" . $device_id . "',`status` = '1' ,`category_type_id`= '".$category_type_id."',`profile_photo` = '".$profile_photo."' "  ;
				$stmt = $dbCon->prepare($sql);  
				$stmt->execute();
				$user = new stdClass();
				$user->id = $dbCon->lastInsertId();
...
{
    $sql_user = "SELECT user_id,firstname,lastname,email_address,device_id,join_date,social_id,login_type,device_type,status,role,category_id,profile_photo,category_type_id FROM app_users WHERE user_id='" . $user_id . "' ";
    try {
        $dbCon = getConnection();
        $stmt  = $dbCon->query($sql_user);
        $stmt->execute();
...
function getSingleVendorUser($slug_name,$check_user_id) 
{
    ...
    try {
        ...
        $users                = $stmt->fetchObject();
         ...
         //for user follow
        $sql = $sql_test =  "SELECT id FROM vendor_followers WHERE user_id = '" . $check_user_id . "' AND vendor_id = '" . $users->user_id . "'";
		$stmt                 = $dbCon->query($sql);
		...
		if(!empty($review_array)){
			foreach ($review_array as $review_array_value) {
				...
				$review_array_value->comment = strip_tags($review_array_value->comment);
				...
				$review_array_value->profile_photo =$user_array->profile_photo ;
...
function getSingleUser($user_id)
{
    ...
    try {
	   ...
	   if(!empty($followers_array)){
        ...
        $stmtn              		 = $dbCon->query($sql);
        $document_array 			 = $stmtn->fetchAll(PDO::FETCH_OBJ);
	
	   if(!empty($document_array)){
			
			foreach($document_array as $document_value){
				
				$document_value->document_path = IMG_URL.'uploads/'.$document_value->filename;
		
			}
	   
			$users->document_array = $document_array;
	   
	   }else{

			$users->document_array = array();
		...
		if($users->role == 'Vendor'){
			...
			$sql_query = "SELECT AVG(ur.ratings) AS rating FROM user_review AS u LEFT JOIN user_ratings  AS ur ON ur.vendor_id = u.vendor_id WHERE u.vendor_id = '".$users->user_id."'";
			$stmt   = $dbCon->query($sql_query);
			$rating_array  = $stmt->fetchObject();
			...
			if(!empty($rating_array)){
				...
				if(!empty($rating_array->rating))
					$users->rating = round($rating_array->rating, 1);
				else
					$users->rating = 0;
...
//    $second_opinion_charge = $req->params('second_opinion_charge');
    ...
    $sql .= "`practicing_since` = '" . $practicing_since . "',
			...
			`second_opinion_charges` = '" . $second_opinion_charge . "',
...
function getLocationSubareas($location_id)
{
    ...
    try {
        ...
        $extra_key     = (object) array(
			...
			array_unshift($subarea, $extra_key);
...
function getLocationAll()
{
    ...
    try {
        ...
        $extra_key     = (object) array(
							"id" => '0',
							"location_name"=>'-Select Name-',
							"status" => ''
						
							);
	array_unshift($location, $extra_key);
